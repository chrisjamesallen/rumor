/*
* lgpc.c
* polygon library for Lua 5.2 based on gpc
* Luiz Henrique de Figueiredo <lhf@tecgraf.puc-rio.br>
* 09 May 2012 22:22:48
...
#define MYNAME "gpc"
#define MYVERSION                                                                        \
    MYNAME " library for " LUA_VERSION " / May 2012 / "                                  \
...
static int Ldiff( lua_State *L ) { return Pclip( L, GPC_DIFF ); }

static int Lintersect( lua_State *L ) { return Pclip( L, GPC_INT ); }

static int Lxor( lua_State *L ) { return Pclip( L, GPC_XOR ); }

static int Lunion( lua_State *L ) { return Pclip( L, GPC_UNION ); }
...
static int Ladd( lua_State *L ) /** add(p,c,[hole]) */
{
    ...
    for ( i = 0; i < n; i++ ) {
        ...
        c.vertex[i].x = lua_tonumber( L, -1 );
        lua_rawgeti( L, 2, 2 * i + 2 );
        c.vertex[i].y = lua_tonumber( L, -1 );
        lua_pop( L, 2 );
    }
...
static const luaL_Reg R[] = { { "__add", Lunion }, /** __add(p,q) */
                              { "__gc", Lgc },
...
LUALIB_API int luaopen_gpc( lua_State *L ) {
    ...
    lua_pushvalue( L, -2 );
    lua_settable( L, -3 );
    lua_setglobal( L, "gpc" ); // note pops top from stack
    return 1;
}
