...
static int vec3_mix(lua_State *L);
...
static int vec3_mix(lua_State *L) {
    ...
    lua_vec3 *vec3R = vec3_userdatap(L, -2);
float t = lua_tonumberx(L, -1, NULL);

lua_vec3 *vec3 = vec3_create(L);
kmVec3 v;
kmVec3Subtract(&v, vec3R->data, vec3L->data);
kmVec3Scale(&v,&v,t);
kmVec3Add(vec3->data, vec3L->data, &v);
...
static int vec3_scale(lua_State *L) {
    ...
    kmVec3Scale(v1->data, v1->data, scale); /** Scales a vector to length s */
    return 1;
};
