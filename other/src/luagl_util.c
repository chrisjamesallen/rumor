...
#define LUAGL_INIT_ARRAY1(parray, type, size, conversionFunc)           \
{                                                               \
    int i;                                                        \
    for(i = 0; i < (size); i++) {                                 \
    lua_rawgeti(L, index, i+1);                                 \
    (parray)[i] = (type)(conversionFunc)(L, -1, NULL);                 \
    lua_remove(L, -1);                                          \
    }                                                             \
}
...
#define LUAGL_INIT_ARRAY21(parray, type, size1, size2, conversionFunc)  \
{                                                               \
int i, j;                                                     \
for(i = 0; i < size1; i++)  {                                 \
lua_rawgeti(L, index, i+1);                                 \
...
for(j = 0; j < *size2; j++) {                               \
lua_rawgeti(L, -1, j + 1);                                \
(parray)[i*(*size2) + j] = (type)(conversionFunc)(L, -1, NULL);  \
lua_remove(L, -1);                                        \
}                                                           \
lua_remove(L, -1);                                          \
}                                                             \
...
#define DEFINE_GET_ARRAY_FUNCx(name, type, conversionFunc) \
int name(lua_State *L, int index, type **parray)           \
{                                                         \
int n = lua_rawlen(L, index);                           \
*parray = LUAGL_NEW_ARRAY(type, n);                           \
LUAGL_INIT_ARRAY1(*parray, type, n, conversionFunc);           \
return n;                                              \
}
...
#define DEFINE_GET_ARRAY_POINTER_FUNC1(name, type, conversionFunc) \
int name(lua_State *L, int index, type **parray)           \
{                                                         \
int n = lua_rawlen(L, index);                           \
*parray = LUAGL_NEW_ARRAY_POINTER(type, n);                   \
...
#define DEFINE_GET_ARRAY2_FUNC1(name, type, conversionFunc)   \
int name(lua_State *L, int index, type **parray, int *size)   \
{                                                            \
int n = lua_rawlen(L, index);                              \
lua_rawgeti(L, index, 1);                                 \
...
*size = lua_rawlen(L, -1);                                 \
*parray = LUAGL_NEW_ARRAY2(type, n, *size);                      \
LUAGL_INIT_ARRAY21(*parray, type, n, size, conversionFunc);       \
lua_remove(L, -1);                                        \
return n;                                                 \
}
...
#define DEFINE_GET_ARRAY2_POINTER_FUNC1(name, type, conversionFunc)   \
int name(lua_State *L, int index, type **parray, int *size)   \
{                                                            \
int n = lua_rawlen(L, index);                              \
lua_rawgeti(L, index, 1);                                 \
...
DEFINE_GET_ARRAY_FUNCx(luagl_get_arrayd, double, lua_tonumberx)
DEFINE_GET_ARRAY_FUNCx(luagl_get_arrayf, float, lua_tonumberx)
DEFINE_GET_ARRAY_FUNCx(luagl_get_arrayi, int, lua_tointegerx)
DEFINE_GET_ARRAY_FUNCx(luagl_get_arrayui, unsigned int, lua_tointegerx)
DEFINE_GET_ARRAY_FUNCx(luagl_get_arrayubyte, unsigned char, lua_tointegerx)

DEFINE_GET_ARRAY_POINTER_FUNC(luagl_get_arrayb_pointer, unsigned char, lua_toboolean)
DEFINE_GET_ARRAY_POINTER_FUNC1(luagl_get_arrayd_pointer, double, lua_tonumberx)

DEFINE_GET_ARRAY2_FUNC1(luagl_get_array2ubyte, unsigned char, lua_tointegerx)
DEFINE_GET_ARRAY2_FUNC1(luagl_get_array2d, double, lua_tonumberx)
DEFINE_GET_ARRAY2_FUNC1(luagl_get_array2f, float, lua_tonumberx)

DEFINE_GET_ARRAY2_POINTER_FUNC1(luagl_get_array2d_pointer, double, lua_tonumberx)
