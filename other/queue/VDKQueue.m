...
//
//  Based heavily on UKKQueue, which was created and copyrighted by Uli Kusterer on 21 Dec
//  2003.
//
...
- (VDKQueuePathEntry *)addPathToQueue:(NSString *)path notifyingAbout:(u_int)flags {
    @synchronized( self ) {
        ...
        if ( pathEntry ) {
            ...
            // Start the thread that fetches and processes our events if it's not already
            // running.
...
//
//  WARNING: This thread has no active autorelease pool, so if you make changes, you must
//  manually manage
...
- (void)watcherThread:(id)sender {
    int n;
    struct kevent ev;
    struct timespec timeout = { 1, 0 }; // 1 second timeout. Should be longer, but we need
                                        // this thread to exit when a kqueue is dealloced,
                                        // so 1 second timeout is quite a while to wait.
    int theFD = _coreQueueFD;           // So we don't have to risk accessing iVars when the thread
                                        // is terminated.
            ...
            if ( n > 0 ) {
                ...
                if ( ev.filter == EVFILT_VNODE ) {
                    ...
                    if ( ev.fflags ) {
                        ...
                        //
                        //  Note: VDKQueue gets tested by thousands of CodeKit users who
                        //  each watch several thousand files at once.
                        //        I was receiving about 3 EXC_BAD_ACCESS (SIGSEGV) crash
                        //        reports a month that listed the 'path' objc_msgSend
                        //        as the culprit. That suggests the KEVENT is being sent
                        //        back to us with a udata value that is NOT what we
                        //        assigned
                        //        to the queue, though I don't know why and I don't know
                        //        why it's intermittent. Regardless, I've added an extra
                        //        check here to try to eliminate this (infrequent)
                        //        problem. In theory, a KEVENT that does not have a
                        //        VDKQueuePathEntry
                        //        object attached as the udata parameter is not an event
                        //        we registered for, so we should not be "missing" any
                        //        events. In theory.
                        //
                        id pe = ev.udata;
                        if ( pe && [pe respondsToSelector:@selector( path )] ) {
                            NSString *fpath = [( (VDKQueuePathEntry *)pe ).path retain]; // Need to
                                                                                         // retain so it
                                                                                         // does not
                                                                                         // disappear
                                                                                         // while the
                            ...
                            NSArray *notes = [[NSArray alloc] initWithArray:notesToPost]; // notesToPost will be changed
                                                                                          // in the next loop iteration,
                                                                                          // which will likely occur
                                                                                          // before the block below
                                                                                          // runs.


                            // Post the notifications (or call the delegate method) on the
                            // main thread.
                            dispatch_async( dispatch_get_main_queue(), ^{
                                for ( NSString *note in notes ) {
                                    ...
                                    if ( !_delegate || _alwaysPostNotifications ) {
                                        ...
                                        [[[NSWorkspace sharedWorkspace] notificationCenter] postNotificationName:note
                                                                                                          object:self
...
- (void)addPath:(NSString *)aPath {
    ...
    @synchronized( self ) {
        ...
        if ( !entry ) {
            ...
            if ( !entry ) {
                NSLog( @"VDKQueue tried to add the path %@ to watchedPathEntries, but "
                       @"the VDKQueuePathEntry was nil. \nIt's possible that the host " @"process has hit its max open file descriptors limit.",
                       aPath );
            }
        }
    }
