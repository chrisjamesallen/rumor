...
function SVG:extractHull(svgStr)
    print("\n\nSvg:Extract Hull:\n", svgStr, "\n")
    -- extract hull
    self:extractOperations(svgStr)
    self.data_index = self.data_index -1
    _.pop(self.paths) --will always have an extra path redundant
    self:triangulate()
    return self.data_
end

function SVG:extractCurves(svgStr)
    print("\n\nSvg:Extract Hull:\n", svgStr, "\n")
    -- extract hull
    self:extractOperations(svgStr)
    self.data_index = self.data_index -1
...
function SVG:extractOperations(svgStr)
    local operations = {}
    self:openSubPath()
...
function SVG:convertSvgToPoints(operations)
    ...
    -- so lets create the basic hull
    -- create a hull by taking
    ...
    _.each(operations, function(i)
        ...
        -- Cubic curveto C (c)
        if i.command == 'c' then
--            _.push(self.data_, {  i[1] + self.pos.x,  i[2] + self.pos.y, 0 })
            if(i[5] < 0) then
                --goto continue
            end
            --_.push(self.data_, {  i[3] + self.pos.x,  i[4] + self.pos.y, 0 })
...
--            _.push(self.data_, {  self.pos.x ,  self.pos.y , 0 })
             _.push(self.data_, {  self.pos.x + i[3],  self.pos.y + i[4], 0 })
            self.pos.x = self.pos.x + i[3]
            self.pos.y =  self.pos.y + i[4]
        end
        if i.command == 'S' then
--           _.push(self.data_, {  self.pos.x,  self.pos.y, 0 })
             _.push(self.data_, {  i[3],  i[4], 0 })
           self.pos.x = i[3]
           self.pos.y =  i[4]
        end
...
function SVG:pointilize()
    self.data_ = _.flatten(self.paths)
    self.vertices = math.ceil(#self.data_ / 3) -- this is cause there are three points to each vertex
