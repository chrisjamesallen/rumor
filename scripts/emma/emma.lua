...
function Em:setVertexArray()
    ...
    str = "M105.1,194c-54.7,0-87.4-36-87.4-89.6l30.9,2.9C49.3,151,77.2,169,109.5,169L105.1,194z"

    local points = svg:extract(str);
    self.vertsCount = svg.vertices
    self.meshes = {}
    self.meshes.points = {}
    self.meshes.points.data = array.new(points, svg.points)

    local data = self.meshes.points.data
    self.meshes.points.ref = gl.CopyData(data, array.len(data));
    gl.BufferData(gl.ARRAY_BUFFER, 4 * 3 * array.len(data), self.meshes.points.ref, gl.DYNAMIC_DRAW);
...
--    -- draw data
--    gl.PointSize(2)
--    gl.DrawArrays(gl.POINTS, 0, self.vertsCount)--)
